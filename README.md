# 🛍️ 쇼핑몰 개발 프로젝트 (Shop Application Project)
* 전자상거래의 주요 기능을 개발하는데 목표를 둔 쇼핑몰 프로젝트입니다.
...
## 6. 기능 구현
📌 [기능 동작 영상 바로가기](https://youtu.be/lYs4cKJ-WVk)
...
## 8. 개발 후기
* 학습했던 JPA와 Spring Data JPA를 실제 프로젝트에 적용하여 쇼핑몰 기능을 구현하면서 웹 애플리케이션의 주요 기능을 구현하는 연습을 할 수 있었습니다.
* 이전 프로젝트에서는 사용하지 않았던 Spring Security를 이번 프로젝트에 적용하여, Spring Security가 제공하는 다양한 기능을 직접 사용해보고 보안이 강화된 쇼핑몰을 개발할 수 있었습니다.
* 데이터 조회 시 동적 처리를 위해 QueryDSL을 도입하여, 복잡한 SQL문 대신 JPA로 동적 쿼리를 코드로 구현할 수 있었습니다.
* OAuth 2.0을 적용한 SNS 계정 연동 로그인 기능을 구현하면서, Spring Security의 이해도를 높이고 사용자 편의성을 향상시킬 수 있었습니다.
* JSP 대신 Thymeleaf를 적용하여 순수 HTML을 유지하며 백엔드 개발에 집중할 수 있었고, 이번에 학습한 Thymeleaf 문법을 통해 추후 다른 프로젝트에서도 활용할 수 있을 것 같습니다.
* JPA를 처음 적용했던 이전 프로젝트보다 더 깊이 있게 JPA를 이해한 상태에서 개발하여, 엔티티 간의 연관관계 매핑 등을 체계적으로 진행할 수 있었습니다. 엔티티 설계에 필요한 어노테이션의 종류와 특징을 파악하고 사용하여, 이전에 MyBatis를 사용했던 프로젝트보다 더 효율적으로 프로젝트를 진행할 수 있었습니다.
* 이번 프로젝트에서는 API 반환 타입을 대부분 페이지 리다이렉트와 연관하여 개발하였는데, 다음 프로젝트에서는 이전에 개발했던 도서관리 애플리케이션 프로젝트처럼 JSON으로 반환하도록 API 설계를 진행하면 좋을 것 같다고 생각했습니다.
* 이번 쇼핑몰 프로젝트에서 경험한 기능 개발을 바탕으로, 추후 진행할 프로젝트에서는 좀 더 자신감을 가지고 다양한 기능을 구현할 수 있을 것 같습니다. 이번 프로젝트 경험을 살려, 앞으로는 더 완성도 있는 프로젝트를 진행하고 싶습니다. ☺️

...
## 6. 기능 구현
[🖥️ 기능 동작 영상 바로가기](https://youtu.be/lYs4cKJ-WVk)
...
## 8. 개발 후기
* 따로 학습했던 JPA, Spring Data JPA 를 직접 적용하여 쇼핑몰 기능을 구현하면서 웹사이트의 주요 기능을 적용하는 연습을 할 수 있었습니다.
* 이전 프로젝트에서 사용하지 않았던 Spring Security 를 프로젝트에 적용하면서, Spring Security가 제공하는 기능을 사용해보고, 보안 부분에서 강화된 프로젝트를 진행할 수 있었습니다.
* 데이터 조회를 진행할 때, 동적 처리를 하기 위해서 QueryDSL 을 도입하여 JPA 로 동적 쿼리를 복잡한 SQL문이 아닌 코드로 구현할 수 있었습니다. 
* 이전부터 프로젝트에 적용하고 싶었던 OAuth 2.0을 적용한 SNS 계정 연동 로그인 기능을 구현하면서, Spring Security의 이해도를 높일 수 있었고, 사용자 편의성을 높일 수 있었습니다. 
* 이전 프로젝트에서는 JSP 대신 Thymeleaf를 적용하면서 순수 HTML 을 유지하며 백엔드쪽 개발에 집중하는데 초점을 뒀는데, 이번에 학습한 Thymeleaf 문법을 통해 추후 다른 프로젝트에 활용할 수 있을 것 같습니다. 
* JPA를 처음 적용했던 프로젝트보다 JPA 이해도가 있는 상태에서 개발해서 엔티티간의 연관관계 매핑 등을 좀 더 체계적으로 진행할 수 있었습니다. 엔티티 설계에 필요한 어노테이션의 종류, 특징 등을 파악하고 사용해서 설계 부분에서 이전에 Mybatis 를 사용했던 프로젝트보다 더 효율적으로 프로젝트를 진행할 수 있었습니다. 
* 추후 진행할 프로젝트에서는 이번 쇼핑몰 프로젝트에서 경험한 기능 개발 경험을 토대로 다른 기능을 개발할 때 자신감이 붙을 것 같습니다. 이번 프로젝트 경험을 살려 추후 프로젝트에서는 좀 더 완성도 있는 프로젝트를 진행하고 싶습니다. ☺️
